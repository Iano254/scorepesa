<div class="strip margin-mobi">
    <div class="heading-t">
        <span class="fa fa-soccer-ball-o" style="margin-right:5px; color:white"></span>
        <span style="color:white;">HIGHLIGHTS</span>
    </div>
    <div class="form-t" style="display:flex; align-items: left; padding-left:15px; padding-right:15px; padding-top:8px;">
        <div class="input-f" >
            <div class="dropdown"style="margin-right:15px; ">
                <button style="border-radius:0; background-color:white; color:black; border-color:white" class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Today Games
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="#">Action</a>
                    <a class="dropdown-item" href="#">Another action</a>
                    <a class="dropdown-item" href="#">Something else here</a>
                </div>
                
            </div>
            <div class="dropdown"style="margin-right:15px; ">
                <button style="border-radius:0; background-color:white; color:black; border-color:white" class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Leagues
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="#">Action</a>
                    <a class="dropdown-item" href="#">Another action</a>
                    <a class="dropdown-item" href="#">Something else here</a>
                </div>
                
            </div>
            <div class="dropdown"style="margin-right:15px; ">
                <button style="border-radius:0; background-color:white; color:black; border-color:white" class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Country
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="#">Action</a>
                    <a class="dropdown-item" href="#">Another action</a>
                    <a class="dropdown-item" href="#">Something else here</a>
                </div>
                
            </div>

            
            
        </div>
    </div>
</div>

<div class="odds">
    

    <div class="odd-sec" style="margin-bottom:10px;">
        <div class="meta-sec" style="display:flex; justify-content: space-between;padding-left:15px; padding-right:15px; padding-top:10px; padding-bottom:10px;">
            <div class="league">
                <span style="font-weight:bold;">PREMIERE LEAGUE</span>
            </div>
            <div class="action">
            <a href="./more.php"><span class="fa fa-plus" style=""></span></a>
            </div>
        </div>
            
            <div class="row">

                <div class="col-md-2" style="padding:0px 5px;">
                    <table style="width:100%">
                        <tr>
                            <td style="padding-left:10px;">
                                <span class="meta-t green-txt">14:30 26/07/19 ID: 1306</span><br>
                                <span class="meta-t">UNITED vs CITY</span><br>
                                <span class="meta-t green-txt">0 : 0</span><br>

                            </td>
                            
                        </tr>
                    </table>
                </div>
                <div class="col-md-3 margin-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper">
                        <table class="" style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">HOME</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">DRAW</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">AWAY</span></td>
                        </tr>
                        </table>
                        <table class="" style="width:100% ">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; margin-right:1px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff; background-color:#101721">3.60</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                    </div>  
                </div>
                <div class="col-md-3 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper">
                    <table class="" style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">1ORX</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">XOR2</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">1OR2</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; margin-right:1px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff; ">3.60</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                    </div>  
                </div>
                <div class="col-md-2 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper" style="">
                        <table style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">OVER</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">UNDER</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                        
                    </div>  
                </div>
                <div class="col-md-2 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper" style="">
                        <table style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">GG</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">NG</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                        
                    </div>  
                </div>
                
                
            </div>

            
        
        
    
    </div>





    <div class="odd-sec" style="margin-bottom:10px;">
        <div class="meta-sec" style="display:flex; justify-content: space-between;padding-left:15px; padding-right:15px; padding-top:10px; padding-bottom:10px;">
            <div class="league">
                <span style="font-weight:bold;">PREMIERE LEAGUE</span>
            </div>
            <div class="action">
            <a href="./more.php"><span class="fa fa-plus" style=""></span></a>
            </div>
        </div>
            
            <div class="row">

                <div class="col-md-2" style="padding:0px 5px;">
                    <table style="width:100%">
                        <tr>
                            <td style="padding-left:10px;">
                                <span class="meta-t green-txt">14:30 26/07/19 ID: 1306</span><br>
                                <span class="meta-t">UNITED vs CITY</span><br>
                                <span class="meta-t green-txt">0 : 0</span><br>

                            </td>
                            
                        </tr>
                    </table>
                </div>
                <div class="col-md-3 margin-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper">
                        <table class="" style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">HOME</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">DRAW</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">AWAY</span></td>
                        </tr>
                        </table>
                        <table class="" style="width:100% ">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; margin-right:1px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">3.60</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                    </div>  
                </div>
                <div class="col-md-3 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper">
                    <table class="" style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">1ORX</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">XOR2</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">1OR2</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; margin-right:1px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff; background-color:#101721">3.60</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                    </div>  
                </div>
                <div class="col-md-2 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper" style="">
                        <table style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">OVER</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">UNDER</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                        
                    </div>  
                </div>
                <div class="col-md-2 hide-mobi" style=" padding:0px 5px;">
                    <div class="odd-selector-wrapper" style="">
                        <table style="width:100%">
                        <tr>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">GG</span></td>
                            <td style="text-align:center;"><span class="htts" style="text-align:center;">NG</span></td>
                        </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="odd-selector" style="border-radius:7px; border-right:1px solid #fff;">2.91</td>
                                <td class="odd-selector" style="border-radius:7px; border-left:1px solid #fff;">2.23</td>
                            </tr>
                        </table>
                        
                    </div>  
                </div>
                
                
            </div>

            
        
        
    
    </div>

    
</div>




